...
//Task 4: (project) Create a site that can display songs from an artist specified by a user search.
    ...
    $("#searchButton").click(function(){
        ...
        console.log( "You searched " + userSearch)
        
        for (var years in library.music){
            for (var artist in library.music[years]){
                console.log(artist, userSearch)
                
                if (userSearch === artist){
                    // console.log("Hello")
                    $("results").append(library.music[years][artist][0].song)
                }
            }
        }
